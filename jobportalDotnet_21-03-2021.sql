...
as
begin
select * from jobseeker where jsid=@jsid
end
...
alter proc sp_jobrecruiter_insert
...
alter proc sp_jobrecruiter_select
as
begin
select * from jobrecruiter
join tblcountry on jobrecruiter.country=tblcountry.cid
join tblstate on jobrecruiter.state=tblstate.sid
join tblcity on  jobrecruiter.city=tblcity.ctid
where status=0
end
...
as
begin
select * from jobseeker where email=@email and password=@password
end


---select * from jobseeker where email='fahadsid8026@gmail.com' and password='zeeshan12'
...
as
begin
update jobseeker set password=@new
where jsid=@id and password=@old
end